...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    if (!invitesLoading && pendingInvites.length > 0) {
      ...
      const shownInvites = JSON.parse(
        sessionStorage.getItem("shownInvites") || "[]"
      );

      if (!shownInvites.includes(firstInvite.roomId)) {
        setCurrentInvite(firstInvite);
        setInviteModalOpen(true);
      }
    }
  }, [user, invitesLoading, pendingInvites]); 
  const handleAccept = async () => {
    if (!currentInvite) return;
    try {
      await acceptInvite(currentInvite.roomId, user.uid);
      toast.success(
        `You've joined the "${currentInvite.projectName}" project!`
      );
      setPendingInvites((invites) =>
        invites.filter((inv) => inv.roomId !== currentInvite.roomId)
      );
      ...
      sessionStorage.setItem(
        "shownInvites",
        JSON.stringify([...shownInvites, currentInvite.roomId])
      );
    } catch (error) {
